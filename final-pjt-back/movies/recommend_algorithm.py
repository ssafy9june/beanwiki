...
def recommend(search):
    ...
    data = data[['id', 'title', 'genres', 'vote_average', 'vote_count', 'popularity','keywords']]
    ...
    c_vector_genres = count_vector.fit_transform(data['keywords'])
    ...
    js = []
    # [0] id, [1] title, [2] gernes, [3] vote_average, [4], vote_count, [5] popularity, [6] keywords, [7] score 
    for i in ans:
        js.append({'id' : i[0],'title': i[1], 'vote_average': i[3], 'vote_count' : i[4], 'popularity': i[5],'score' : i[7]})
    # print(fi - st)
    # return json.dumps(js,ensure_ascii=False)
    return js

def csv_recommend(search):
    data = pd.read_csv('clicks.csv', encoding='utf-8')
    data = data[['id', 'title', 'genres', 'vote_average', 'vote_count', 'popularity','keywords']]
